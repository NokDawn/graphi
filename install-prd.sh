...
echo -e '\n'
SHOW_WARNING "
Please, add the lines below in your /etc/hosts file (superuser will be necessary) \n
127.0.0.1   api.local.ths.com \n
"

if [ ! -f .env ]; then
    SHOW_H1 "Creating .env file"
    if (cp .env_example_prd .env); then
        SHOW_SUCCESS ".env file successfully created!"
    else
        SHOW_ERROR "I was not able to create the .env file, please check the errors above!"
    fi
...
if [ ! -f docker-compose.yml ]; then
    ...
    if (cp docker-compose-prd.yml docker.compose.yml); then
        SHOW_SUCCESS "docker-compose.yml file successfully created!"
    else
        SHOW_ERROR "I was not able to create the docker-compose.yml file, please check the errors above!"
    fi

    SHOW_WARNING "Some variables in the docker-compose.yml files should be filles, please take a look at that"
fi
